...
import { enableCSSParsing, nativeHTMLMap } from '../g-lite/globalContants/contants'
  ...
  apply(context: RenderingPluginContext) {
    ...
    const handleMounted = (e: FederatedEvent) => {
      ...
      if (object.nodeName === Shape.HTML) {
        if (!this.$camera) {
          this.$camera = this.createCamera(camera) as any
        }

        // create DOM element
        const $el = this.getOrCreateEl(object as any)
    ...
    renderingService.hooks.init.tapPromise(HTMLRenderingPlugin.tag, async () => {
      ...
      canvas!.addEventListener(ElementEvent.MOUNTED, handleMounted as any)
      canvas!.addEventListener(ElementEvent.UNMOUNTED, handleUnmounted as any)
      canvas!.addEventListener(ElementEvent.ATTR_MODIFIED, handleAttributeChanged as any)
      canvas!.addEventListener(ElementEvent.BOUNDS_CHANGED, handleBoundsChanged as any)
    })
    ...
    renderingService.hooks.destroy.tap(HTMLRenderingPlugin.tag, () => {
      ...
      canvas!.removeEventListener(ElementEvent.MOUNTED, handleMounted as any)
      canvas!.removeEventListener(ElementEvent.UNMOUNTED, handleUnmounted as any)
      canvas!.removeEventListener(ElementEvent.ATTR_MODIFIED, handleAttributeChanged as any)
      canvas!.removeEventListener(ElementEvent.BOUNDS_CHANGED, handleBoundsChanged as any)
    })
  }
    ...
    switch (name) {
      ...
      case 'height':
        if (enableCSSParsing) {
          ...
          $el.style.height = isNumber(height) ? `${height}px` : height.toString()
        }
        break
      case 'zIndex':
