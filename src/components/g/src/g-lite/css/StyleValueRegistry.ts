...
import { CSSKeywordValue } from './cssom/CSSKeywordValue';
...
import { geometryUpdaterFactory } from '../globalContants/geometryConstants';
import { CSSPropertySyntaxFactory } from '../globalContants/cssPropertyConstants';

const EMPTY_PARSED_PATH = {
...
export class DefaultStyleValueRegistry implements StyleValueRegistry {
  ...
  getPropertySyntax(syntax: string) {
    return CSSPropertySyntaxFactory[syntax];
  }
  ...
  processProperties(
    ...
    if (!enableCSSParsing) {
      ...
      if (attributes.textTransform) {
        CSSPropertySyntaxFactory['<text-transform>'].calculator(
          null,
          null,
      ...
      if (attributes.clipPath) {
        CSSPropertySyntaxFactory['<defined-path>'].calculator(
          'clipPath',
          oldClipPath,
          attributes.clipPath,
          object,
      ...
      if (attributes.markerStart) {
        // @ts-ignore
        object.parsedStyle.markerStart = CSSPropertySyntaxFactory[
          '<marker>'
        ].calculator(
          null,
          // @ts-ignore
      ...
      if (attributes.markerEnd) {
        // @ts-ignore
        object.parsedStyle.markerEnd = CSSPropertySyntaxFactory[
          '<marker>'
        ].calculator(
          null,
          // @ts-ignore
      ...
      if (attributes.markerMid) {
        // @ts-ignore
        object.parsedStyle.markerMid = CSSPropertySyntaxFactory[
          '<marker>'
        ].calculator(
          '',
          // @ts-ignore
      ...
      if (
        ...
        CSSPropertySyntaxFactory['<coordinate>'].postProcessor(
          object,
          attributeNames,
        );
      }
      if (!isNil(attributes.zIndex)) {
        CSSPropertySyntaxFactory['<z-index>'].postProcessor(
          object,
          attributeNames,
        );
      }
      // @ts-ignore
      if (attributes.path) {
        CSSPropertySyntaxFactory['<path>'].postProcessor(
          object,
          attributeNames,
        );
      }
      // @ts-ignore
      if (attributes.points) {
        CSSPropertySyntaxFactory['<list-of-points>'].postProcessor(
          object,
          attributeNames,
        );
      }
      if (!isNil(attributes.offsetDistance)) {
        CSSPropertySyntaxFactory['<offset-distance>'].postProcessor(
          object,
          attributeNames,
        );
      }
      if (attributes.transform) {
        CSSPropertySyntaxFactory['<transform>'].postProcessor(
          object,
          attributeNames,
        );
      }
    ...
    if (enableCSSParsing && object.children.length) {
      attributeNames.forEach((name) => {
  ...
  private updateGeometry(object: DisplayObject) {
    ...
    const geometryUpdater = geometryUpdaterFactory[nodeName];
    if (geometryUpdater) {
