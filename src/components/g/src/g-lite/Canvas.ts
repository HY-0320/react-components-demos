...
import { patchStyleValueRegistry } from './globalContants/styleValueRegistryContant';
import { patchSceneGraph } from './globalContants/sceneGrpahContants';
import { patchConstants, globalThis } from './globalContants/contants';
import { patchTextConstants } from './globalContants/textConstants';
import { patchGeometry } from './globalContants/geometryConstants';
import { patchCSSPropertySyntax } from './globalContants/cssPropertyConstants';
...
export class Canvas extends EventTarget implements ICanvas {
  ...
  constructor(config: CanvasConfig) {
    ...
    this.requestAnimationFrame =
      requestAnimationFrame ?? raf.bind(globalThis);
    this.cancelAnimationFrame =
      cancelAnimationFrame ?? caf.bind(globalThis);
  ...
  private initDefaultCamera(width: number, height: number) {
    ...
    // bind camera
    this.context.camera = camera as any;
  }
  ...
  private async initRenderer(renderer: IRenderer) {
    ...
    patchStyleValueRegistry(runtime)
    patchSceneGraph(runtime)
    patchConstants(runtime)
    patchTextConstants(runtime)
    patchGeometry(runtime)
    patchCSSPropertySyntax(runtime)
