...
import { createVec3, decompose, getEuler, rad2deg, fromRotationTranslationScale } from '../utils/math'
import type { CustomElement } from './CustomElement';
import { enableCSSParsing, globalThis } from '../globalContants/contants';
import { styleValueRegistry } from '../globalContants/styleValueRegistryContant';
...
> extends Element<StyleProps, ParsedStyleProps> {
  ...
  constructor(config: DisplayObjectConfig<StyleProps>) {
    ...
    const Proxy: ProxyConstructor = globalThis.Proxy
      ? globalThis.Proxy
      : function () {};
  ...
  setLocalPosition(position: vec3 | vec2 | number, y = 0, z = 0) {
    sceneGraphService.setLocalPosition(
      this,
  ...
  getEulerAngles() {
    const [, , ez] = getEuler(
      vec3.create(),
      sceneGraphService.getWorldTransform(this),
    );
    return rad2deg(ez);
  }
  ...
  getLocalEulerAngles() {
    const [, , ez] = getEuler(
      vec3.create(),
      sceneGraphService.getLocalRotation(this),
    );
    return rad2deg(ez);
  }
