...
import {offscreenCanvas as globalOffscreenCanvas} from '../../../g-lite/globalContants/textConstants'
...
export class DefaultRenderer implements StyleRenderer {
  ...
  private getPattern(
    ...
    if ((pattern.image as Rect).nodeName === 'rect') {
      ...
      $offscreenCanvas = globalOffscreenCanvas.getOrCreateCanvas(
        offscreenCanvas!
      ...
      const offscreenCanvasContext = globalOffscreenCanvas.getOrCreateContext(
        offscreenCanvas!
      ) as CanvasRenderingContext2D

      const restoreStack = [] as any[]

      // offscreenCanvasContext.scale(1 / dpr, 1 / dpr);

      ;(pattern.image as Rect).forEach((_object: DisplayObject) => {
