...
import type { PathGenerator } from '../g-plugin-canvas-path-generator';
...
export class CanvasRendererPlugin implements RenderingPlugin {
  ...
  apply(context: RenderingPluginContext, runtime: GlobalRuntime) {
    ...
    renderingService.hooks.init.tapPromise(
      ...
      async () => {
        ...
        canvas!.addEventListener(ElementEvent.CULLED, handleCulled as any);
    ...
    renderingService.hooks.destroy.tap(CanvasRendererPlugin.tag, () => {
      ...
      canvas!.removeEventListener(ElementEvent.CULLED, handleCulled as any);
      // this.renderQueue = [];
  ...
  private searchDirtyObjects(dirtyRectangle: AABB): DisplayObject[] {
    ...
    return rBushNodes.map(({ displayObject }: any) => displayObject);
  }
